...
class BaseForm(ABC):
    ...
    def __init_(self, browser, timeout = DEFAULT_TIMEOUT):
    ...
    def wait_until_invisible(self, locator, timeout=None):
        timeout = timeout or self.timeout
        try:
            self.logger.info(f"Waiting for element invisible: {locator}")
    ...
    def find_element_by_text(self, elements, text):
        self.logger.info(f"Searching for text '{text}' in elements")
...
def submit_and_wait(
    ...
    try:
        self.logger.info(f"Submitting form using button: {button_locator}")
